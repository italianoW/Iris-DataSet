...
def treinar(atributos, rotulos, epocas, taxa_aprendizado):
    ...
    pesos_in_oculta = [[random.uniform(-0.1, 0.1) for _ in range(4)] for _ in range(4)]  # 4x4
    bias_oculta = [random.uniform(-0.1, 0.1) for _ in range(4)]

    out_oculta_pesos = [[random.uniform(-0.1, 0.1) for _ in range(3)] for _ in range(4)]    # 4x3
...
# === Treinamento ===
    ...
    for _ in range(epocas):
        ...
        for entrada, esperado in zip(atributos, rotulos_onehot):
            ...
            entrada_oculta = [bias_oculta[i] + sum(entrada[j]
                                * pesos_in_oculta[j][i] for j in range(4))
                                for i in range(4)]
            ativacao_oculta = u.sigmoid(entrada_oculta)

        # Cálculo da ativação da camada de saída
            entrada_saida = [
            bias_saida[i] + sum(ativacao_oculta[j]
            * out_oculta_pesos[j][i] for j in range(4))
            for i in range(3)]
            ...
            for i in range(3):
                for j in range(4):
                    ...
                    out_oculta_pesos[j][i]-=taxa_aprendizado * erro_saida[i] * ativacao_oculta[j]
                    out_oculta_pesos[j][i]=max(min(out_oculta_pesos[j][i], LIMITE_SUP), LIMITE_INF)
            ...
            erro_oculta = [sigmoid_oculta[i]
                           * sum(erro_saida[k] * out_oculta_pesos[i][k]
                           for k in range(3))
                           for i in range(4)]
...
# Atualização dos pesos da oculta
            for j in range(4):
                for k in range(4):
                    pesos_in_oculta[j][k]-=taxa_aprendizado * erro_oculta[k] * entrada[j]
                    pesos_in_oculta[j][k]=max(min(pesos_in_oculta[j][k], LIMITE_SUP), LIMITE_INF)

            for k in range(4):
                bias_oculta[k] -= taxa_aprendizado * erro_oculta[k]
                bias_oculta[k] = max(min(bias_oculta[k], LIMITE_SUP), LIMITE_INF)

    return pesos_in_oculta, out_oculta_pesos, bias_oculta, bias_saida
